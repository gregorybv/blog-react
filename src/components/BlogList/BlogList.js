...
export const BlogList = memo(({blogs}) => {
    const {blogsLoading, searchBlogsLoading} = useBlogsContext();
    const blogLimit = 6;
    const [paginate, setPaginate] = useState(blogLimit);
    const paginateHandler = useCallback((value) => setPaginate(value * blogLimit), [blogLimit]);
                    ...
                    return (<div className="blog-item" key={blog.id}>
                        ...
                        <div className='blog-item-tags'>
                            {blog.tags.map((tag, idx) => (
                                <span className='blog-item-tags-single fs-13 font-rubik text-uppercase'
                                      key={idx}>{tag}</span>))}
                        </div>
                        <div className='blog-item-btn'>
                            <Link to={`/blog/${blog.id}`} className="read-more-btn font-rubik fw-4">Read
                                More</Link>
                        </div>
                    </div>)
                })}
